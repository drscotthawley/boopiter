...
# %% auto #0
__all__ = ['DEFAULT_TOOL_SELECTION', 'OLLAMA_PREFIX', 'OLLAMA_NUM_CTX', 'OLLAMA_KEEP_ALIVE_MIN', 'OLLAMA_URL', 'get_tool_list',
           'ollama_loaded', 'unload_ollama_model', 'sync_ollama_loaded', 'get_ollama_list', 'get_model_list',
           'tools_system_prompt', 'prompt_llm', 'stream_llm_reply']
...
OLLAMA_NUM_CTX = 32768  # default context window (tokens) requested from Ollama; the GUI overrides it per-notebook (nb.num_ctx, see _num_ctx_control). Ollama otherwise allocates the model's own advertised maximum, and since the KV cache covers the whole window whether you fill it or not, that maximum can dwarf the weights: qwen3-vl:4b is ~3GB of 4.4B-parameter weights but advertises a 262144-token window, which lands it at ~46GB resident. 32768 tokens is roughly 1400 lines of notebook at ~23 tokens/line -- ample for real use -- and matches what qwen3:14b and qwen2.5-coder already default to.
...
def _chat_callkw(model:str, keep_alive_min:int|None=None, num_ctx:int|None=None) -> dict:
    ...
    if model.startswith('ollama'):
        ...
        # num_ctx isn't one of the fields litellm's ollama_chat transformer pulls out by name, so it
        # stays in optional_params and rides along in the request's 'options' dict -- which is exactly
        # where Ollama expects it. See OLLAMA_NUM_CTX for why we set it rather than taking the default.
        kw['num_ctx'] = OLLAMA_NUM_CTX if num_ctx is None else num_ctx
    return kw

def prompt_llm(context:str, model:str=OLLAMA_PREFIX+'qwen2.5-coder:latest', tools:list|None=None, think:str|None=None, keep_alive_min:int|None=None, num_ctx:int|None=None) -> tuple[str,str]:
    ...
    chat = Chat(model, sp=tools_system_prompt(tools), tools=[], callkw=_chat_callkw(model, keep_alive_min, num_ctx)) # FYI: this makes a fresh stateless context each time. is that what we want?
...
def stream_llm_reply(context:str, model:str, tools:list|None=None, think:str|None=None, images:list|None=None, keep_alive_min:int|None=None, num_ctx:int|None=None):
    ...
    if model.startswith('deaddrop/'):
        ...
        return
    chat = Chat(model, sp=tools_system_prompt(tools), tools=[], callkw=_chat_callkw(model, keep_alive_min, num_ctx))
