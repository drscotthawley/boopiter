...
def _write_index(proj:Path, remote:dict) -> None:
    "Write the share site's index page, listing this machine's shares alongside the pages that exist only on gh-pages (see _index_contents). Dropping the ones this machine can render is the whole job of the filter, and it is not optional: a share made here is found by the listing's own glob and also has a sidecar on the branch from the publish that put it there, so any name left in `remote` that has a local source draws a second, identical card. That is what happened the first time a second machine published -- one notebook, two cards."
    local = {d.name for d in proj.iterdir() if (d/'index.ipynb').exists()}
    (proj/'index.qmd').write_text(_INDEX_QMD.format(
        docs=_docs_url(), contents=_index_contents({n: m for n, m in remote.items() if n not in local})))
...
def _publish_additive(proj:Path, add:dict, drop:set) -> None:
    ...
    try:
        ...
        what = ', '.join(sorted(add or drop)) or 'index'   # neither: a re-render to refresh the index alone
        subprocess.run(w + ['commit', '-q', '--allow-empty', '-m', f'Publish {what}'], check=True, timeout=120)
...
def share_notebook(nb_path, listed:bool=True, images_dir=None) -> str:
    ...
    _write_index(proj, remote)   # drops everything renderable here, this share included, so no page is listed twice
