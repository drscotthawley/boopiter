...
This is a semi-vibe-coded clone of, and homage to, the amazing [solveit](https://solve.it.com/) system. `boopiter` is intended primarily for education purposes: learning the basic operations so that I can easily work with local execution and develop my own extensions and features as my research work expands.
...
- [x] Code cell outputs are text-only (with some color support). A real Jupyter notebook can render matplotlib figures, IPython HTML output, and interactive widgets like tqdm progress bars. Would need a proper output-type dispatch (image/html/etc.) instead of assuming text.
